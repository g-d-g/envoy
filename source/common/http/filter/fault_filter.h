...
public:
  ...
  std::vector<std::string>& downstreamNodes() { return downstream_nodes_; }
...
private:
  ...
  std::vector<std::string> downstream_nodes_; // Downstream nodes for which failure is injected.
...
private:
  ...
  void postDelayInjection(const std::string& abort_percent_key,
                          const std::string& abort_http_status_code);
  void abortWithHTTPStatus(const std::string& abort_http_status_key);
